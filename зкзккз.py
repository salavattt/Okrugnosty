...
class Ui_MainWindow(object):
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.pushButton.clicked.connect(self.LineEdit_run)
        self.what_chat = 0

    def LineEdit_run(self):
        if self.lineEdit.text().lower() == 'привет' or self.lineEdit.text().lower() == 'hello':
            self.chat_hello()
        if self.lineEdit.text().lower() == 'как дела?' or self.lineEdit.text().lower() == 'how are you?':
            self.chat_how_are_you()
        if self.lineEdit.text().lower() == 'как тебя зовут?' or self.lineEdit.text().lower() == 'what is your name?':
            self.what_is_your_name()
        if self.lineEdit.text().lower() == 'скажи погоду на завтра' or self.lineEdit.text().lower() == 'weather':
            self.weather()
